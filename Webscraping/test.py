...
def croma_scrapper(url):
    ...
    # Find title
    title_element = soup.find('h1', class_="pdp__title")
    title = title_element.text.strip() if title_element else None
    data['title'] = title
    ...
    # Find ratings and reviews
    value_element = soup.find('span', class_="TextWeb__Text-sc-1cyx778-0 dzLzNm")
    if value_element:
        value = value_element.text.strip()
        split = value.split("&")
        ratings = split[0].split()[0]
    ...
    # # # Find stars
    # stars_element = soup.find('span', class_="")
    # stars = stars_element.text.strip() if stars_element else None
    # data['stars'] = stars

    # # Find price
    price_element = soup.find('span', class_="TextWeb__Text-sc-1cyx778-0 kFBgP")
    price = price_element.text.strip() if price_element else None
    data['price'] = price
    ...
    return data
url='https://www.reliancedigital.in/apple-iphone-15-128gb-blue/p/493839312'
product_data= croma_scrapper(url)
print(product_data)
